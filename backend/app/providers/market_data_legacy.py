...
@dataclass(frozen=True)
class FinancialBundleSnapshot:
    eps_rows: list[EpsSnapshot]
    quarters: list[FinancialQuarterSnapshot]
...
def fetch_financial_bundle(
    ...
    *,
    ...
    return FinancialBundleSnapshot(
        eps_rows=_parse_finmind_eps(normalized_symbol, payload),
        quarters=_parse_financial_quarters(normalized_symbol, payload)[-quarters:],
    )
